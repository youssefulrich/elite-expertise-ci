...
export default function Accueil() {
  ...
  return (
    <div style={{ overflowX: 'hidden' }}>
          ...
          <div className="max-w-2xl">
            ...
            <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(30px)', transition: 'all 0.7s ease 0.3s' }}>
              <h1 className="font-display text-4xl md:text-6xl font-extrabold leading-tight mb-6">
                ...
                <span style={{
                  backgroundImage: 'linear-gradient(90deg, #ff6b6b, #e74c3c, #ff6b6b)',
            ...
            <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.7s ease 0.7s' }}>
              <div className="flex flex-wrap gap-4 mb-10">
                <Link to="/boutique"
                  className="flex items-center gap-2 font-bold px-7 py-3.5 rounded-xl text-white transition-all hover:scale-105 hover:shadow-xl hover:shadow-red-600/50"
                  style={{ background: 'linear-gradient(135deg, #ff6b6b, #e74c3c)' }}>
                  Voir nos produits <ArrowRight size={18} />
                </Link>
                <Link to="/contact"
                  className="flex items-center gap-2 font-bold px-7 py-3.5 rounded-xl border-2 border-white text-white hover:bg-white hover:text-red-600 transition-all hover:scale-105">
                  <Phone size={18} /> Demander un devis
                </Link>
              </div>
              <div className="flex flex-wrap gap-5">
                {['Produits certifiés', 'Livraison Abidjan', 'SAV inclus'].map((t, i) => (
                  <span key={t} className="flex items-center gap-2 text-sm text-gray-200"
                    style={{ animation: `slideIn 0.5s ease ${0.8 + i * 0.15}s both` }}>
                    <CheckCircle size={15} className="text-red-400" /> {t}
                  </span>
                ))}
              </div>
            </div>
      ...
      {/* ===== BARRE MARQUES ===== */}
      <section className="py-5 overflow-hidden" style={{ background: '#e74c3c' }}>
        <div className="flex items-center gap-6 px-6">
          <span className="text-red-100 text-xs font-bold uppercase tracking-widest shrink-0">Nos marques</span>
          <div className="overflow-hidden flex-1">
            <div className="marquee-track gap-4">
              {[...marques, ...marques].map((m, i) => (
                <span key={i} className="text-white text-sm font-bold px-5 py-2 rounded-full shrink-0 mx-2 cursor-default"
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((s, i) => (
            <AnimSection key={s.label} delay={i * 100}>
              <div className="text-center p-6 rounded-2xl border-2 hover:shadow-lg transition-all hover:-translate-y-1"
                style={{ borderColor: '#e74c3c', background: i % 2 === 0 ? '#e74c3c' : '#fff' }}>
                <p className="font-display font-extrabold text-5xl" style={{ color: i % 2 === 0 ? '#fff' : '#e74c3c' }}>
                  <Counter target={parseInt(s.val)} suffix={s.val.includes('+') ? '+' : ''} />
                  {s.val.includes('ans') ? ' ans' : ''}
                </p>
                <p className="text-sm mt-1 font-medium" style={{ color: i % 2 === 0 ? '#fecaca' : '#6b7280' }}>{s.label}</p>
              </div>
            </AnimSection>
          ))}
        </div>
      ...
      <section className="py-20 px-6" style={{ background: '#fff5f5' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {categories.map(({ nom, desc, slug, img, icon: Icon }, i) => (
      ...
      {(loadingProduits || produitsVedette.length > 0) && (
        <section className="py-20 px-6 bg-white">
          <div className="max-w-7xl mx-auto">
            ...
            {loadingProduits ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                {[...Array(4)].map((_, i) => <div key={i} className="bg-red-50 rounded-2xl h-64 animate-pulse" />)}
              </div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                {produitsVedette.map((p, i) => (
                  <AnimSection key={p.id} delay={i * 100}>
                    <Link to={`/boutique/${p.slug}`}
                      className="bg-white rounded-2xl hover:shadow-xl transition-all duration-300 group overflow-hidden hover:-translate-y-1 block"
      ...
      {/* ===== GALERIE TERRAIN ===== */}
      <section className="py-20 px-6" style={{ background: '#e74c3c' }}>
        <div className="max-w-7xl mx-auto">
          <AnimSection className="text-center mb-12">
            <span className="inline-block bg-white/20 text-white text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-3">Sur le terrain</span>
            <h2 className="font-display text-3xl font-extrabold text-white mb-2">Nos équipements en action</h2>
            <p className="text-red-100">Des solutions déployées dans les plus grandes entreprises de Côte d'Ivoire</p>
          </AnimSection>
      ...
      <section className="py-20 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <AnimSection className="flex items-center justify-between mb-10">
            ...
            <Link to="/realisations" className="hidden md:flex items-center gap-1.5 text-sm font-bold hover:gap-3 transition-all text-red-600">
              Voir tout <ArrowRight size={16} />
            </Link>
          </AnimSection>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {partenaires.map((p, i) => (
              <AnimSection key={p.nom} delay={i * 80}>
                <div className="flex items-center gap-4 p-5 rounded-2xl bg-white border-2 hover:shadow-lg transition-all duration-300 group hover:-translate-y-0.5"
                  style={{ borderColor: '#fca5a5' }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = '#e74c3c'}
                  onMouseLeave={e => e.currentTarget.style.borderColor = '#fca5a5'}>
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 font-extrabold text-white text-lg shadow group-hover:scale-110 transition-transform"
                    style={{ background: 'linear-gradient(135deg, #ff6b6b, #e74c3c)' }}>
                    {p.nom[0]}
                  </div>
                  <div>
      ...
      {/* ===== TÉMOIGNAGES ===== */}
      <section className="py-20 px-6" style={{ background: '#C0392B' }}>
        <div className="max-w-7xl mx-auto">
          <AnimSection className="text-center mb-12">
            <span className="inline-block bg-white/20 text-white text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-3">Avis clients</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {temoignages.map((t, i) => (
              <AnimSection key={t.nom} delay={i * 120}>
                <div className="bg-white rounded-2xl p-6 relative hover:shadow-2xl transition-all duration-300 group hover:-translate-y-1"
                  style={{ border: '2px solid rgba(255,255,255,0.3)' }}>
                  <Quote size={32} className="absolute top-4 right-4 text-red-200" />
                  <div className="flex gap-1 mb-4">
                    {[...Array(t.note)].map((_, i) => <Star key={i} size={14} fill="#e74c3c" style={{ color: '#e74c3c' }} />)}
                  </div>
                  <p className="text-gray-600 text-sm leading-relaxed mb-5 italic">"{t.texte}"</p>
                  <div className="flex items-center gap-3 border-t border-red-100 pt-4">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0"
                      style={{ background: 'linear-gradient(135deg, #ff6b6b, #e74c3c)' }}>{t.nom[0]}</div>
                    <div>
                      <p className="font-bold text-gray-800 text-sm">{t.nom}</p>
                      <p className="text-red-400 text-xs">{t.poste}</p>
                    </div>
                  </div>
                </div>
              </AnimSection>
      ...
      {/* ===== CERTIFICATIONS ===== */}
      <section className="py-14 px-6 bg-white border-t-4" style={{ borderColor: '#e74c3c' }}>
        <div className="max-w-7xl mx-auto">
          <AnimSection>
            <p className="text-center text-xs font-bold uppercase tracking-widest text-red-500 mb-8">Agréments & normes respectées</p>
            <div className="flex justify-center mb-8">
              <div className="flex items-center gap-5 px-8 py-5 rounded-2xl border-2 hover:shadow-xl transition-all"
                ...
                <div>
                  ...
                  <p className="text-xs font-bold mt-1 flex items-center gap-1 text-red-600">Côte d'Ivoire <CheckCircle2 size={12} /></p>
                </div>
              </div>
            </div>
            <div className="flex flex-wrap justify-center gap-4">
              {certifications.map((c) => (
                <div key={c.code} className="flex items-center gap-3 px-5 py-3 rounded-xl border-2 bg-white hover:shadow-md transition-all hover:-translate-y-0.5 cursor-default"
                  style={{ borderColor: '#fca5a5' }}
                  ...
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #ff6b6b, #e74c3c)' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {pourquoiNous.map(({ icon: Icon, titre, desc }, i) => (
              <AnimSection key={titre} delay={i * 120}>
                <div className="p-7 rounded-2xl bg-white border-2 hover:shadow-xl transition-all duration-300 group hover:-translate-y-1"
                  style={{ borderColor: '#fca5a5' }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = '#e74c3c'}
                  onMouseLeave={e => e.currentTarget.style.borderColor = '#fca5a5'}>
                  ...
                  <h3 className="font-bold text-gray-900 text-base mb-2">{titre}</h3>
      ...
      <section className="relative py-24 px-6 text-center text-white overflow-hidden"
        style={{ background: 'linear-gradient(135deg, #ff6b6b 0%, #e74c3c 40%, #C0392B 100%)' }}>
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(4)].map((_, i) => (
        ...
        <AnimSection className="relative z-10">
          <span className="inline-block bg-white/20 text-white text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-6">Contact</span>
